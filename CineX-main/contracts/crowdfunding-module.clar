...
(define-constant WITHDRAWAL-FEE-PERCENT u3) ;; Platform takes a 3% fee from funds when a campaign owner withdraw 
...
(define-public (claim-campaign-funds (campaign-id uint) (escrow-address <crwd-escrow-trait>))
  (let
    (
      ...
      (fee-amount (/ (* current-total-raised WITHDRAWAL-FEE-PERCENT) u100)) ;; deduction of 3% withdrawal-fee from total-raised money
      (withdraw-amount (- current-total-raised fee-amount)) ;; campaign-owner withdraws total-raised funds minus deduction of 3% 
