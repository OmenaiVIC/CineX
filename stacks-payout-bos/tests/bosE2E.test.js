...
import { verifyHmac, verifyWebhook } from '../src/webhookVerifier.js';
import { createFallbackPoller, pollTarget, mapExternalStatus, isEligible } from '../src/fallbackPoller.js';
import { buildTimeline, formatTimelineText, STATE_LABELS } from '../src/auditTimeline.js';
import { EVIDENCE_TYPES, recordEvidence, recordApiResponse, recordTxHash, recordGateResult, getEvidence } from '../src/evidenceCollector.js';
...
describe('webhookVerifier', () => {
  ...
  it('verifyXReserveWebhook passes with no secret configured', () => {
    ...
    const result = verifyWebhook('xreserve', '{}', {}, ctx);
  ...
  it('verifyYellowCardWebhook returns missing_signature when secret is set', () => {
    const ctx = { config: { yellowCardWebhookSecret: 'yc-secret' }, logger };
    const result = verifyWebhook('yellowcard', '{}', {}, ctx);
  ...
  it('verifyWebhook returns error for unknown source', () => {
    const ctx = { config: {}, logger };
    const result = verifyWebhook('unknown', '{}', {}, ctx);
    expect(result.valid).toBe(false);
...
describe('fallbackPoller', () => {
  ...
  it('maps burn_failed to fail', () => {
    ...
    expect(r.reason).toBe('burn_submitted_failed');
  });
  ...
  it('createFallbackPoller starts and stops', async () => {
    ...
    const poller = createFallbackPoller({ db: mockDb, adapters: mockAdapters, pipelineWorker: mockPipeline, logger: mockLogger, config: {} });
    poller.start();
...
describe('auditTimeline', () => {
  ...
  it('attaches matching snapshots to timeline entries', async () => {
    const db = mockDb({
      ...
      snapshots: [
        { external_system: 'stacks', status: 'confirmed', raw_response: {}, captured_at: '2026-01-01T00:05:02Z' },
      ],
    });
