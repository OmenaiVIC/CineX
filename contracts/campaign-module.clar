...
;; version: 1.0.0
;; implements: .campaign-module-traits.crowdfunding-trait
;; Author: Victor Omenai 
;; Created: 2025

;;;; ============= Description ==============
;; Implementation of Campaign module trait
...
;; Implementing the campaign trait (interface) to follow expected rules
(impl-trait .campaign-module-traits.crowdfunding-trait) 
...
(use-trait crwd-escrow-trait .milestone-escrow-trait.milestone-escrow-trait)
(use-trait crwd-verification-trait .project-verification-module-trait.project-verification-trait)
...
;; Set the core contract (allowed to control this module), as well as the 
;; campaign and escrow contract addresses
...
(define-read-only (get-module-name) 
    (ok "campaign-module") ;; return current module name
)
